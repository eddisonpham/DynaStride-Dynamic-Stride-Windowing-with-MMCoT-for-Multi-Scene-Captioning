"""Semantic evaluation metrics for video captioning."""
import os
import subprocess
import sys

def install_dependencies():
    """Install required dependencies for semantic metrics."""
    commands = [
...
def compute_nsp(tok, mdl, groups: Dict[str, List[Tuple[str, str, str]]], device: str):
    """Compute Next Sentence Prediction metrics."""
    mdl.eval().to(device)
...
def compute_nli_contradiction_rate(tok, mdl, groups: Dict[str, List[Tuple[str, str, str]]], device: str) -> float:
    """Compute NLI contradiction rate."""
    mdl.eval().to(device)
    total, contrad = 0, 0
    for _, seq in groups.items():
        preds = [p for _, _, p in seq if p]
...
def evaluate_file(args):
    """Worker function for evaluating a single file."""
...
def main():
    """Main entry point for semantic metrics evaluation."""
    results_dir = "SemanticResults"
    os.makedirs(results_dir, exist_ok=True)
    
    outputs_dir = "outputs"
    output_pattern = "videollama3"
    json_files = sorted(glob.glob(os.path.join(outputs_dir, output_pattern, "*.json")))
    
    if not json_files:
        json_files = sorted(glob.glob(os.path.join(outputs_dir, "*.json")))
    
    if not json_files:
        print(f"No JSON files found in {outputs_dir}/{output_pattern} or {outputs_dir}.")
        sys.exit(1)
